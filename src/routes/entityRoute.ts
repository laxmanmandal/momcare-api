...
import {
  entityCreateMultipartSchema,
  entityIdParamsSchema,
  entityUpdateMultipartSchema,
  validateData,
  zodToSwagger,
} from "../validations";
...
export default async function entityRoutes(app: FastifyInstance) {
  app.get(
    ...
    async (req: any, reply) => {
      try {
        const payload = await entityService.getChannelEntity(
  ...
  app.get(
    ...
    async (req: any, reply) => {
      try {
        const payload = await entityService.getPartnerEntity(
          req.user.belongsToId,
        );
        reply.code(200).send({
          success: true,
  ...
  app.get(
    "/:id",
    {
      ...
      schema: {
        ...
        params: zodToJsonSchema(entityIdParamsSchema as any, {
          target: "openApi3",
        }),
  ...
  app.get(
    ...
    async (req: any, reply) => {
      try {
        const payload = await entityService.getAllentities(
          req.user.belongsToId,
        );
        reply.code(200).send({
          success: true,
  ...
  app.post(
    "/create",
    {
      ...
      schema: {
        ...
        body: zodToSwagger(entityCreateMultipartSchema),
        response: { 201: successObjectResponse },
      },
    },
    async (req: any, reply: FastifyReply) => {
      const parsed = await app.parseMultipartMemory(req);
      const { fields: body, files } = validateData(
        entityCreateMultipartSchema,
        parsed
      );
      const createdBy =
        (typeof (req.body ?? parsed.fields)?.id === "number" ? (req.body ?? parsed.fields).id : null) ??
        (req as any).user?.id;
      const belongsToId =
  ...
  app.patch(
    "/:id/update",
    {
      preHandler: [authMiddleware, app.accessControl.check("UPDATE_ENTITY")],
      schema: {
        tags: ["Entities"],
        ...
        body: zodToSwagger(entityUpdateMultipartSchema),
        params: zodToJsonSchema(entityIdParamsSchema as any, {
          target: "openApi3",
        }),
    ...
    async (req: any, reply: FastifyReply) => {
      ...
      if (files.imageUrl?.length) {
        body.imageUrl = await app.saveFileBuffer(
          files.imageUrl[0],
          `Entities`,
        );
      }
