...
const ROLE_KEYS = Object.keys(Role); // e.g. ['SUPER_ADMIN', 'MANAGER', ...]
const ROLE_KEY_SET = new Set<string>(ROLE_KEYS);
const ROLE_VALUE_TO_KEY = new Map<string | number, string>(); // maps value -> key
for (const k of ROLE_KEYS) {
    const v = (Role as any)[k];
    // store both string uppercased and number (if numeric)
    ROLE_VALUE_TO_KEY.set(String(v).toUpperCase(), k);
    if (typeof v === "number") ROLE_VALUE_TO_KEY.set(v, k);
}

export function resolveRoleName(roleInput: unknown): string {
    ...
    if (typeof roleInput === "object" && !Array.isArray(roleInput)) {
        const maybe = (roleInput as any).role ?? (roleInput as any).name ?? (roleInput as any).value;
    ...
    if (typeof roleInput === "number") {
        ...
        if (!key) throw new Error(`Unknown role: ${roleInput}`);
        return key;
    }

    // Normalize string-ish input
    const raw = String(roleInput).trim();
    if (!raw) throw new Error(`Unknown role: ${roleInput}`);

    // 1) Direct key match (case-sensitive or case-insensitive)
    if (ROLE_KEY_SET.has(raw)) return raw;
    const upper = raw.toUpperCase();
    if (ROLE_KEY_SET.has(upper)) return upper;
...
export async function generateCustomId(role: Role | string): Promise<string> {
...
export function canCreateRole(
    actorRoleInput: unknown,
    targetRoleInput: unknown,
    opts?: { strict?: boolean }
): boolean {
