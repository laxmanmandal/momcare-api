 ...
 */
export function prismaDuplicateField(err: unknown): string | null {
    ...
    if (msg) {
        ...
        if (detailMatch) return detailMatch.split('.').pop()!.toLowerCase();

        const backtickMatch = msg.match(/`([^`]+)`/);
        if (backtickMatch?.[1]) return backtickMatch[1].toLowerCase();
...
function cleanupConstraintToken(token: string): string | null {
    ...
    if (/^[a-z0-9_]+$/.test(last)) {
        ...
        if (!last.endsWith('_key')) return last;
    }
 ...
 */
export function handlePrismaDuplicate(err: unknown, reply: FastifyReply) {
...
export class ResourceOwnershipError extends PermissionError {
