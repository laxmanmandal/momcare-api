// src/config/parserConfigs.ts
import { ParserConfig } from '../types/excel';

export const CONCIEVE_PARSER_CONFIG: ParserConfig = {
  tableName: 'Concieve',
...
export const ENTITY_PARSER_CONFIG: ParserConfig = {
  ...
  fieldValidators: {
    type: {
      validate: (value: any) =>
        value &&
        ['CHANNEL', 'PARTNER', 'ORGANIZATION'].includes(
    ...
    phone: {
      validate: (value: any) =>
        value && /^\d{10}$/.test(value.toString().trim()),
      message: 'Phone must be 10 digits'
    }
  },
  fieldTransformers: {
    type: { transform: (v: any) => (v ? v.toString().trim().toUpperCase() : null) },
    name: { transform: (v: any) => (v ? v.toString().trim() : null) },
    email: { transform: (v: any) => (v ? v.toString().trim().toLowerCase() : null) },
    phone: { transform: (v: any) => (v ? v.toString().trim() : null) },
    location: { transform: (v: any) => (v ? v.toString().trim() : null) },
    description: { transform: (v: any) => (v ? v.toString().trim() : null) },
    imageUrl: { transform: (v: any) => (v ? v.toString().trim() : null) },
    createdBy: { transform: (v: any) => (v ? parseInt(v) : null) },
    belongsToId: { transform: (v: any) => (v ? parseInt(v) : null) },
    isActive: {
      transform: (v: any) =>
        v === null || v === undefined
          ? true
...
export const USER_PARSER_CONFIG: ParserConfig = {
  tableName: "User",
  ...
  fieldTransformers: {
    uuid: {
      transform: (v) =>
        v ? v.toString().trim() : undefined, // let DB generate if missing
    },
    role: {
...
export const PARSER_CONFIGS: Record<string, ParserConfig> = {
  concieve: CONCIEVE_PARSER_CONFIG,
  user: USER_PARSER_CONFIG,
  EntityTable: ENTITY_PARSER_CONFIG, // canonical
