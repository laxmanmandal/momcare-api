...
export default fp(async function (fastify: FastifyInstance) {
  ...
  function loadConfig() {
    ...
    if (!fs.existsSync(rolesFile) || !fs.existsSync(apisFile)) {
      ...
      apiMatchers = [];
      console.warn(
        "[AccessControl] ⚠️ Config files not found. Access control will deny by default.",
      );
      return;
    }

    // ✅ Load roles (normalize role + policies)
    const rolesRaw = JSON.parse(
      fs.readFileSync(rolesFile, "utf8"),
    ) as RolesFileEntry[];

    rolesMap = rolesRaw.reduce(
      (acc, r) => {
        const key = String(r.roleName).trim().toUpperCase();

        acc[key] = (r.policy || []).map((p) =>
    ...
    const apis = JSON.parse(
      fs.readFileSync(apisFile, "utf8"),
    ) as ApiFileEntry[];

    apiMatchers = apis.map((a) => ({
      apiId: a.apiId,
      policyNames: (a.policyName || []).map((p) =>
        String(p).trim().toUpperCase(),
      ),
      matcher: match<Record<string, string>>(a.apiEndpoint, {
    ...
    console.log(
      `[AccessControl] ✅ Loaded roles: ${Object.keys(rolesMap).join(", ")}`,
    );
  ...
  fastify.decorate("accessControl", {
    ...
    check: (requiredPolicy: string) => {
      return async (req: FastifyRequest) => {
        ...
        if (!allowed) {
          const err = createHttpError(403, `Role ${role} not configured`);
          (err as any).code = "ROLE_NOT_FOUND";
          throw err;
        }
