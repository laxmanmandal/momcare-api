...
import { CouponValidationResult, PriceCalculationInput } from '../types/coupon';
...
export async function getCouponByCode(coupon_code: string) {
...
export async function CouponStatus(id: number, reply: any) {
...
export async function calculateFinalPrice(input: {
    couponCode?: string;
    userId: number;
    planId: number;
}) {
    try {
        ...
        const fixedAmount =
            coupon.fixed_amount !== null ? Number(coupon.fixed_amount) : null;
 ...
 */
export async function validateCoupon(couponCode?: string, entityId?: number): Promise<CouponValidationResult> {
    try {
        let coupon = null;
        ...
        if (isValid && coupon.used_count > 0) {
            const usageCount = await prisma.planAllocation.count({
                where: {
 ...
 */
export async function getUserCoupons(entityId?: number) {
    try {
        const now = new Date();
