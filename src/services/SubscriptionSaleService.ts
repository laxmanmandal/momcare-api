...
export async function createPlanAllocation(input: any, user: any) {
  ...
  const type = String(input.type).toUpperCase() as
    | "ALLOCATE"
    | "SELL"
    | "REVOKE";
  ...
  const receiverId =
    input.receiverId !== undefined ? Number(input.receiverId) : null;
  ...
  return await prisma.$transaction(async (tx) => {
    ...
    if (!isOrgAdmin(user) && type === "SELL") {
      if (!senderId)
        throw new Error(
          "SenderId required for SELL (senderId or user.belongsToId)",
        );
...
export async function createPaymentOrder(
  ...
  if (calculation.finalAmount !== amount) {
    throw new Error(
      `Amount mismatch, could be altered! Expected ${calculation.finalAmount}, got ${razorpayOrder.amount},parameter amount ${amount} `,
    );
  }
