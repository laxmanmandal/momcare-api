...
exports.getPlansByManyIds = getPlansByManyIds;
exports.getPlan = getPlan;
exports.createPlan = createPlan;
exports.updatePlan = updatePlan;
exports.SubscriptionStatus = SubscriptionStatus;
exports.SubscriptionVisible = SubscriptionVisible;
...
async function getPlansByManyIds(ids) {
    const plans = await prisma.subscriptionPlan.findMany({
        where: {
            id: {
                in: ids,
...
async function createPlan(data) {
    ...
    const { courseIds, ...planFields } = data;
...
async function updatePlan(uuid, data) {
    ...
    return await prisma.$transaction(async (tx) => {
        ...
        if (planFields.thumbnail && plan.thumbnail && plan.thumbnail !== planFields.thumbnail) {
            try {
                await (0, fileUploads_1.deleteFileIfExists)(plan.thumbnail);
...
async function SubscriptionStatus(uuid) {
...
async function SubscriptionVisible(uuid) {
    const plan = await prisma.subscriptionPlan.findUnique({ where: { uuid } });
    if (!plan)
        throw new Error("Subscription plan not found");
    return prisma.subscriptionPlan.update({
